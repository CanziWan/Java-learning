...
public class ATMSystem {
    public static void main(String[] args) {
        // 1、定义账户类
        // 2、定义一个集合容器，负责存储以后存储全部的账户对象，进行相关的业务操作
        ArrayList<Account> accounts = new ArrayList<>();
        // 3、展示系统的首页
        Scanner sc = new Scanner(System.in);
        while (true) {
            System.out.println("=================TUNGEE-ATM系统=================");
    ...
    private static boolean deleteAccount(Account acc, Scanner sc, ArrayList<Account> accounts) {
        ...
        System.out.println("您是否真的需要销户？Y/N");
        String rs = sc.next();
        switch (rs) {
            case "Y":
                // 真正的销户
    ...
    private static void transferMoney(Scanner sc, Account acc, ArrayList<Account> accounts) {
        ...
        // 2、判断自己的账户是否有钱
        if (acc.getMoney() == 0) {
            System.out.println("对不起，您的账户余额为" + acc.getMoney() + "元，无法进行转账操作！");
            return; // 结束当前方法
        }
        ...
        while (true) {
            ...
            if (account == null) {
                ...
                // 这个账户对象存在了：继续认证他的姓氏
                String userName = account.getUserName();    // 张三
    ...
    private static String getRandomCardId(ArrayList<Account> accounts) {
        ...
        while (true) {
            ...
            Account acc = getAccountByCardId(cardId, accounts);
            if (acc == null) {
                // 说明cardId 此时没有重复，这个卡号是一个新卡号，可以使用这个卡号作为新注册账户的卡号
                return cardId;
            }
        }
    }
